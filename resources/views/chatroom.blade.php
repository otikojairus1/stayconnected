...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ALL GROUP POSTS | VIEWED BY ONLY TEAM MEMBERS
        </h2>
    </x-slot>
    ...
    <div class="md:flex ml-5 animate__animated animate__backInLeft">
        ...
        <div class="ml-5 flex">
            ...
            <a href="/create/group/posts">  <div class="mt-5 ml-4 py-3 px-5 mb-4 bg-green-100 text-green-900 text-sm rounded-md border border-green-200" role="alert">
                CREATE GROUP POST
             </div></a>

             <a href="/group/members/{{\App\Models\Group::where('userid',Auth::id())->first()->competitionid}}">  <div class="mt-5 ml-4 py-3 px-5 mb-4 bg-green-100 text-green-900 text-sm rounded-md border border-green-200" role="alert">
                VIEW GROUP MEMBERS
             </div></a>
                ...
                <div class="w-full md:w-9/12 mx-2 h-64 animate__animated animate__backInRight">
                    ...
                    <div class="bg-green-lightest my-2 border-l-4 border-green text-green-dark p-4" role="alert">
                        <div class="flex">
                            ...
                            GROUP<p class="font-bold ml-2 mr-3">{{$message->competitionid}} </p>POST 
                            <p class="font-bold mr-3"> <br/>{{$message->content}} </p>
                            posted by <p class="ml-3">{{App\Models\User::find($message->userid)->lastname}}<p class="ml-2">{{\App\Models\User::find($message->userid)->firstname}}</p>
                        </div>
                        
                        <p class="font-bold">posted at {{$message->created_at}}</p>
                        
                            <form action="/groups/delete/{{$message->id}}" method="POST">
                                @csrf
